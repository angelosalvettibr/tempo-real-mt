...
.ctx-perg input{flex:1;min-width:200px;font-family:'Source Serif 4',Georgia,serif;font-size:15.5px;
  border:1px solid #CBD9E6;background:#fff;padding:11px 13px}
.ctx-perg input:focus{outline:none;border-color:#1F4D7A}
.ctx-perg button{font-family:'IBM Plex Mono',monospace;font-size:10.5px;letter-spacing:.12em;text-transform:uppercase;
  background:#1F4D7A;color:#fff;border:0;padding:11px 17px;cursor:pointer}
.ctx-perg button:disabled{opacity:.5;cursor:default}
  ...
  if (bc) bc.addEventListener('click', function(){
    ...
    fetch('/api/contexto', { method:'POST', headers:{'Content-Type':'application/json'},
      ...
      .then(function(j){
        ...
        // A caixa de pergunta aparece SEMPRE — inclusive quando o agente
        // decide que nao ha o que explicar. Antes disso, quem ficava com
        // duvida ficava sem saida.
        montarPergunta();
    ...
    var ir = function(){
      ...
      fetch('/api/contexto', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ acao:'perguntar', pergunta:q, titulo:t,
          ...
          nivel: sg && sg.dataset ? sg.dataset.nivel : '',
          id: location.pathname.split('/').pop().replace(/\.html$/,'') }) })
        .then(function(r){ return r.json(); })
        .then(function(j){
          bt.disabled = false; bt.textContent = 'perguntar';
          if (j.ok) {
            sa.innerHTML = '<div class="ctx-resp"><div class="q">' + q.replace(/[<>]/g,'') + '</div>'
              + j.paragrafos.map(function(p){ return '<p>' + p.replace(/[<>]/g,'') + '</p>'; }).join('')
              + (j.fontes ? '<div class="fontes">Levantado em: ' + j.fontes.join(' · ').replace(/[<>]/g,'') + '</div>' : '')
              + '</div>';
            document.getElementById('cp-q').value = '';
          } else {
            sa.innerHTML = '<div class="ctx-resp"><p>' + (j.msg || 'não consegui responder').replace(/[<>]/g,'') + '</p></div>';
          }
        })
